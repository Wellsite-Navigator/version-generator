import {
  ...
  processReleases,
  DEFAULT_MAJOR_VERSION_INCREMENT,
  getPlayStoreVersionInfo,
  AndroidExecutor,
...
describe('Android Version Code', () => {
  ...
  describe('getPlayStoreVersionInfo', () => {
    ...
    it('should handle base64-encoded service account key', async () => {
      ...
      // Verify that parseJSONValue correctly handles the base64 key
      const parsedValue = parseJSONValue(base64Key);
      expect(parsedValue).toEqual({ test: 'value' });

      // Test the full function with the base64 key
    ...
    it('should propagate errors from the executor', async () => {
      const mockExecutor = new MockAndroidExecutor(100, 2);
      mockExecutor.setError('Test error from executor');
      await expect(getPlayStoreVersionInfo('com.example.app', '{}', undefined, mockExecutor)).rejects.toThrow(
        'Test error from executor',
      );
    });
  });
