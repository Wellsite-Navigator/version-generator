...
describe('iOS Version', () => {
  ...
  describe('getIosBuildNumberInfo', () => {
    ...
    it('should throw an error when required options are missing', async () => {
      await expect(
        getIosBuildNumberInfo({ ...baseOptions, apiKeyId: undefined }, mockExecutor)
      ).rejects.toThrow('iOS build number generation requires apiKeyId, apiIssuerId, apiPrivateKey, bundleId, and appReleaseVersion');
      expect(mockExecutor.generateToken).not.toHaveBeenCalled();
      expect(mockExecutor.queryApi).not.toHaveBeenCalled();
    });
    ...
    it('should handle API errors gracefully', async () => {
      ...
      await expect(getIosBuildNumberInfo(baseOptions, mockExecutor))
        .rejects.toThrow('Failed to get iOS build number');
    });
  });
  ...
  describe('getIosBuildNumber', () => {
    beforeEach(() => {
      ...
      jest.spyOn(require('./ios-version'), 'getIosBuildNumber').mockImplementation(
        async (...args: unknown[]) => {
  ...
  describe('getAppStoreVersionInfo', () => {
    ...
    it('should find the highest build number when multiple builds match the filter', async () => {
      ...
      // Verify that our mock executor methods were called with the correct parameters
      expect(multipleBuildsExecutor.generateToken).toHaveBeenCalledWith('test-key-id', 'test-issuer-id', 'test-private-key');
    ...
    it('should handle base64-encoded private key', async () => {
      ...
      expect(mockExecutor.generateToken).toHaveBeenCalledWith(
        'test-key-id', 
        'test-issuer-id', 
        'test-private-key'
      );
    ...
    it('should handle API errors gracefully', async () => {
      ...
      await expect(getAppStoreVersionInfo(
        'com.example.app',
        '1.0.0',
        'test-key-id',
        'test-issuer-id',
